...
Before that, we will change the column names for the labels and subject datasets, because they are both named "V1" and this same name is used in the X-test dataset.
...
We know that the first column in the df_mean_std dataset is named "subject" , the second is named "label" and the remaining, with the actual measurement data, goes from "V1" to "V561".
...
In this step, we will group the data for label and subject, obtaining the mean by using dplyer package again and write the tidy file.
